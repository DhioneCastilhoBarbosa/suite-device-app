...
import { pcdFirmware } from '../PcdPluviometrica'
...
export function Update({ isConect }: UpdateProps): JSX.Element {
  ...
  const portName = pcdFirmware.getPortName()
  const portConnected = isConect && pcdFirmware.isPortOpen()
  ...
  const handleCheckRecovery = async (): Promise<void> => {
    ...
    try {
      ...
      const result = await pcdFirmware.checkRecovery()
      if (result.ok) {
  ...
  const handleSelectFile = async (): Promise<void> => {
    ...
    const result = await window.pluvidbUpdater.selectFile(t('Firmware PCD Pluviométrica'))
  ...
  const handleFlash = async (): Promise<void> => {
    ...
    try {
      await pcdFirmware.upload(firmwarePath, (off, total) => {
