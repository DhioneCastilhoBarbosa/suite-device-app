...
export default function UpdateModubus(){
  ...
  const synced =()=>{
    ...
    setStatus(status=> status +'Sincronizado com o sensor concluida!\nAtualização em andamento aguarde... \n')
  ...
  const handleUpdate = ()=>{
    ...
    setShowModal(false);
    atualizaFirmware({file:fileContent, portName:port.name, baudRate:baudRateSelect, synced,fail,finished})
  ...
  <div className="flex flex-col items-center  w-full h-96 ">
    <div className='flex flex-col w-2/3 mt-8'>
      <label>Status</label>
      <textarea className=" h-36 leading-relaxed border border-[2px] border-zinc-200 resize-none whitespace-pre-wrap outline-none text-black text-sm rounded-md pl-4 pt-4"
         value={status}
         readOnly
      />
